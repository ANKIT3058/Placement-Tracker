import {
  ...
  upsertStudentProfileRegistrationNumber,
...
type NormalizedRegistrationNumber =
...
// The caller's student profile, as the API exposes it.
//
// `registrationNumber` and nothing else: the row's `id` and `userId` are
// internal and are deliberately not returned. Exposing the profile id would
// offer a second way to name the record, and the only supported way to reach it
// is to be its owner.
...
export const updateStudentProfileService = async (
  ...
  try {
    const profile = await upsertStudentProfileRegistrationNumber(
      context.userId,
      registrationNumber,
    );
...
const isRegistrationNumberConflict = (error: unknown): boolean => {
  if (
    typeof error !== "object" ||
    error === null ||

...
import type {
  ...
  SpreadsheetSheet,
  SpreadsheetRow,
} from "./types/spreadsheet-data.interface.js";
...
// only parses.
export class SpreadsheetParser implements AttachmentParser {
  ...
  async parse(filePath: string): Promise<ParsedAttachment> {
    ...
    const sheets: SpreadsheetSheet[] = workbook.worksheets.map((worksheet) =>
      this.extractSheet(worksheet),
    );

    const structuredData: SpreadsheetData = { sheets };
  ...
  private extractSheet(worksheet: Worksheet): SpreadsheetSheet {
  ...
  // aggressive cleaning.
  private buildText(sheets: SpreadsheetSheet[]): string {
    return sheets
      .map((sheet) => {

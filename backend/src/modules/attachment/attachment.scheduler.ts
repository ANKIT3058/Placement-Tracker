...
import {
  ...
  ATTACHMENT_RECONCILE_MIN_AGE_MS,
  ATTACHMENT_RECONCILE_BATCH_SIZE,
...
const runReconcileCycle = async (): Promise<void> => {
  if (isRunning) {
    console.warn(
      "[attachment-reconciler] Previous sweep still in progress, skipping",
    );
    return;
  }
  ...
  try {
    ...
    console.error(
      "[attachment-reconciler] Sweep failed",
...
export const startAttachmentReconciliationScheduler = (): void => {
  if (timer) {
    console.warn(
      "[attachment-reconciler] Already started, ignoring duplicate start",
    );
    return;
  }

  console.log(
    `[attachment-reconciler] Starting, interval ${ATTACHMENT_RECONCILE_INTERVAL_MS}ms`,
  );

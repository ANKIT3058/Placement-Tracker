...
const resolveRound = (stage: unknown): string | null => {
  if (typeof stage !== "string") return null;

  const normalized = stage.trim().toLowerCase();

  if (normalized.length === 0 || normalized === UNRESOLVED_ROUND) return null;

  return normalized;
};

export const classifyRoundIdentity = (
  ...
  if (candidate === null || incoming === null) return "UNKNOWN";
...
export const passesIdentityGate = (relation: IdentityRelation): boolean =>

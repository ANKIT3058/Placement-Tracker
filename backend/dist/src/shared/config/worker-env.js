...
export const EMAIL_WORKER_REQUIRED_ENV = ["DATABASE_URL", "REDIS_URL"];
/**
 ...
 */
export const findMissingEnv = (required, env = process.env) => required.filter((name) => (env[name] ?? "").trim() === "");
/**
 ...
 */
export const resolveRequiredEnv = (base, env = process.env) => env.USE_AI === "true" ? [...base, "OPENAI_API_KEY"] : [...base];
/**
 ...
 */
export const assertWorkerEnv = (worker, base, env = process.env) => {
    const missing = findMissingEnv(resolveRequiredEnv(base, env), env);
    if (missing.length === 0) {
        return;
    }
    console.error(`[${worker}] Fatal configuration error: required environment variable(s) not set: ${missing.join(", ")}`);
    console.error(`[${worker}] Refusing to start. Set the variable(s) above in the unit's EnvironmentFile and restart the service.`);
    process.exit(1);
};
//# sourceMappingURL=worker-env.js.map
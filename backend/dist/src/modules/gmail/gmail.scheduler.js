...
const runSyncCycle = async () => {
    ...
    try {
        ...
        console.log(`[gmail-scheduler] Run started, ${accounts.length} account(s) to sync`);
        let succeeded = 0;
        let failed = 0;
        ...
        for (const account of accounts) {
            try {
                ...
                failed += 1;
                console.error(`[gmail-scheduler] Failed to sync account ${account.email}`, error);
            }
        }
        console.log(`[gmail-scheduler] Run finished, ${succeeded} succeeded, ${failed} failed`);
    }
    catch (error) {
...
export const startGmailScheduler = () => {
    if (timer) {
        console.warn("[gmail-scheduler] Already started, ignoring duplicate start");
        return;
    }
    console.log(`[gmail-scheduler] Starting, interval ${GMAIL_SYNC_INTERVAL_MS}ms`);
...
//# sourceMappingURL=gmail.scheduler.js.map
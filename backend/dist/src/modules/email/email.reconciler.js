 ...
 */
export const reconcilePendingEmails = async ({ olderThan, }) => {
    ...
    for (const email of orphans) {
        try {
            ...
            console.error(`[email-reconciler] Failed to enqueue email ${email.id}`, error instanceof Error ? error.message : "Unknown error");
        }
    }
    return { enqueued };
};
//# sourceMappingURL=email.reconciler.js.map